        ...
        <div className="bg-gradient-to-br from-gray-50 to-slate-100 rounded-3xl p-12 border border-gray-200">
          <div className="grid md:grid-cols-3 gap-8 mb-12">
            {benefits.map((benefit, index) => (
              <div key={benefit.title} className="text-center">
                ...
                <h4 className="text-xl font-black text-gray-900 mb-3 font-heading">{benefit.title}</h4>
                <p className="text-gray-600 leading-relaxed font-medium font-sans">{benefit.description}</p>
              </div>
            ))}
          </div>
          ...
          <div className="text-center">
            <div className="inline-flex flex-col sm:flex-row gap-4">
              <Link to="/signup?role=recruiter" className="px-8 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold rounded-2xl hover:from-blue-700 hover:to-purple-700 transition-all shadow-xl hover:shadow-2xl hover:-translate-y-1 duration-300 font-heading">
                Start Hiring Today
              </Link>
              <Link to="/signup?role=developer" className="px-8 py-4 border-2 border-gray-300 text-gray-700 font-bold rounded-2xl hover:border-gray-400 hover:bg-gray-50 transition-all duration-300 font-heading">
                Find Your Dream Job
              </Link>
            </div>
          </div>
