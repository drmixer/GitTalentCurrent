...
const TestsAndConsole: React.FC<{
  ...
  runKey: string; // Add runKey to force re-render
  ...
  }, [onTestsComplete, runKey]); // Add runKey to dependency array
  return (
    <div style={{ width: '50%', display: 'flex', flexDirection: 'column', borderLeft: '1px solid #e5e7eb' }}>
      <div ref={testsRootRef} style={{ flex: 1, minHeight: 0 }}>
        ...
        <SandpackTests 
          ...
          key={runKey} // Add key to force re-render
        />
      </div>
...
> = (props) => {
  ...
  const [runKey, setRunKey] = useState('initial-run'); // Unique key for each run
  const { sandpack } = useSandpack();
  const { code } = useActiveCode(); // Use useActiveCode hook to get current code
  const testsRootRef = useRef<HTMLDivElement>(null);
  const isRunningRef = useRef(false); // Ref to track running state to avoid race conditions
...
export default defineConfig({
  ...
  const handleRunTests = async () => {
    ...
    try {
      // Generate a new run key to force re-render
      const newRunKey = `run-${Date.now()}`;
      setRunKey(newRunKey);
