...
const TestsAndConsole: React.FC<{
  ...
  isRunning: boolean; // <-- CHANGE #1: Added prop to control the observer
  ...
  useEffect(() => {
    ...
    // If tests aren't running or the component isn't ready, do nothing.
    if (!isRunning || !root) {
    ...
    const obs = new MutationObserver(() => {
      ...
      if (parsed.ran) {
        ...
        // The observer is no longer disconnected here. The cleanup function will handle it.
      }
    });
    ...
    // The cleanup function now runs when `isRunning` becomes false, disconnecting the observer.
    return () => {
...
export default defineConfig({
        ...
        timeoutRef.current = setTimeout(() => {
          setIsRunning((currentIsRunning) => {
            if (currentIsRunning) {
              ...
              alert("The test runner timed out. Please try again.");
              return false;
            }
  ...
  const handleSubmit = async () => {
    ...
    let score = 0;
    if (total && total > 0) {
      score = (typeof failed === 'number' && failed === 0) ? 1 : 0;
    }
    ...
    <>
      ...
      >
        {submitted ? (
          ...
          // CHANGE #2: This is the new UI state for showing results and allowing a rerun.
          (() => {
            const tests = lastParsed?.tests;
          ...
          <>
            ...
            <button
              ...
              disabled={isRunning}
              style={{
                padding: '10px 20px', backgroundColor: isRunning ? '#94a3b8' : '#3b82f6',
                color: 'white', border: 'none', borderRadius: '8px', fontWeight: '600',
                fontSize: '14px', cursor: isRunning ? 'not-allowed' : 'pointer',
