...
function parseSummary(text: string) {
  ...
  // Using . to match any character except newline, which is safer
  const suitesLine = text.match(/Test suites?:(.+)/i)?.[1] ?? text.match(/Test files?:(.+)/i)?.[1] ?? '';
  const testsLine = text.match(/Tests?:(.+)/i)?.[1] ?? '';
  ...
  // Corrected regexes with single backslashes for \d and \s
  ...
  const handleRunTests = async () => {
    ...
    try {
      ...
      if (testButton && testButton instanceof HTMLButtonElement) {
        ...
        setTimeout(() => {
          if (testRunIdRef.current === currentRunId) {
