/*
  # Fix Portfolio Items and Developer Profiles

  1. Adds missing portfolio_items table if it doesn't exist
  2. Adds proper RLS policies for portfolio items
  3. Adds trigger for updated_at timestamp on portfolio items
  4. Fixes developer profile creation function to handle existing profiles
*/
...
-- Create portfolio_items policies
CREATE POLICY IF NOT EXISTS "Public can read portfolio items"
  ON portfolio_items FOR SELECT
  TO anon, authenticated
  USING (true);

CREATE POLICY IF NOT EXISTS "Developers can manage own portfolio items"
