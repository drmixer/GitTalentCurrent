...
CREATE POLICY "Users can send messages"
  ...
  WITH CHECK (
    ...
    (
      ...
      is_admin() OR
      -- Anyone can message admin
      EXISTS (SELECT 1 FROM users WHERE id = receiver_id AND role = 'admin') OR
      -- Recruiter can message assigned developers
      (
        is_recruiter() AND
        EXISTS (
          SELECT 1 FROM assignments a
          WHERE a.recruiter_id = auth.uid() AND a.developer_id = receiver_id
        )
      ) OR
      -- Developer can message assigned recruiters
      (
        EXISTS (SELECT 1 FROM developers WHERE user_id = auth.uid()) AND
        EXISTS (
          SELECT 1 FROM assignments a
          WHERE a.developer_id = auth.uid() AND a.recruiter_id = receiver_id
        )
      )
    )
  );
