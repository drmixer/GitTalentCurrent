/*
  # Fix Migration File - Remove Admin User Creation

  1. Changes
    - Remove admin user creation that's causing conflicts
    - Keep all other database structure and policy changes
    - Maintain all helper functions and triggers
*/
...
-- Log admin status without trying to create one
DO $$
DECLARE
...
BEGIN
  ...
  IF admin_exists THEN
    RAISE NOTICE 'Admin user already exists in the database';
  ELSE
    RAISE NOTICE 'No admin user found in the database';
  END IF;
END $$;