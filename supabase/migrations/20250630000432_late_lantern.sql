...
CREATE TABLE IF NOT EXISTS developers (
  ...
  public_profile_slug text DEFAULT NULL,
  notification_preferences jsonb DEFAULT '{"email": true, "in_app": true, "messages": true, "assignments": true}',
...
BEGIN
  ...
  github_handle := COALESCE(NEW.raw_user_meta_data->>'user_name', '');
...
BEGIN
  ...
  IF profile_exists THEN
    ...
    UPDATE developers SET
      github_handle = p_github_handle,
      bio = p_bio,
      availability = p_availability,
      top_languages = p_top_languages,
      linked_projects = p_linked_projects,
      location = p_location,
      experience_years = p_experience_years,
      desired_salary = p_desired_salary,
      updated_at = now()
